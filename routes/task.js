...
router.post('/', async (req, res) => {
    try {
        let task = await instanciateModelFromRequest(Models.Task, req.body);
...
router.patch('/:id', async (req, res) => {
    try {
        let task = await Models.Task.findByPk(req.params.id);
        if (task !== null) {
            let updatedTask = await updateModelFromRequest(task, req.body);
            let savedTask = await updatedTask.save();
